...
CreateJoinMinerPoint()
{
	//JOBS
	//new strings[128];
	//CreateDynamicPickup(1239, 23, 489.5229,781.7092,-22.0168, -1, -1, -1, 50);
	//format(strings, sizeof(strings), "[MINER JOBS]\n{ffffff}Jadilah Pekerja Miner disini\n{7fffd4}/getjob /accept job");
	//CreateDynamic3DTextLabel(strings, COLOR_YELLOW, 489.5229,781.7092,-22.0168, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1); // Miner
}
...
function MiningOre(playerid)
{
    ...
    if(pData[playerid][MiningOreID] != -1)
	{
		...
		if(pData[playerid][pActivityTime] >= 100 && pData[playerid][MiningOreID] != -1)
		{
			...
			SuccesMsg(playerid, "Anda Berhasil Memining Ore");
			KillTimer(pData[playerid][pActivity]);
...
CMD:ore(playerid, params[])
{
	...
	{
		if(isnull(params)) return SyntaxMsg(playerid, "/ore [info/mine/pickup/sell]");
		...
		if(!strcmp(params, "mine", true))
		{
			...
			if(tid != -1)
			{
				if(!Ore_BeingEdited(tid) && !OreData[tid][oreGettingMiner])
				{
					...
					PlayerTextDrawSetString(playerid, ActiveTD[playerid], "Sedang memecahkan batu...");
		...
		else if(!strcmp(params, "pickup", true))
		{
			...
			SendClientMessage(playerid, COLOR_WHITE,"You've taken a log from ground.");
			RemoveLog(tid);
			// done
		}
		...
		else if(!strcmp(params, "sell", true))
		{
			...
			if (pData[playerid][pJualOreDelay] > 0)
				return Error(playerid, "Please wait %d minutes before selling again.", pData[playerid][pJualOreDelay]);
			...
			SuccesMsg(playerid, "Anda Berhasil Menjual Ore.");
			CrateComponent += LogStorage[vehicleid][0] + 1;
			GasOil += LogStorage[vehicleid][1] + 100;
			pData[playerid][pJualOreDelay] = 15;
			AddPlayerSalary(playerid, "Job (Miner & Oil Driller)", cash);
