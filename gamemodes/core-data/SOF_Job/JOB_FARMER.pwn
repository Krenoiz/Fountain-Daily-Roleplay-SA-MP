...
enum E_PLANT
{
	...
	bool:PlantHarvest,
	PlantTimer,
	STREAMER_TAG_OBJECT:PlantObjID,
	PlantCP,
	Text3D:PlantLabel
}
...
GetClosestPlant(playerid, Float: range = 1.0)
{
...
GetNearPlant(playerid, Float: range = 1.0)
{
	new id = -1, Float: dist = range, Float: tempdist;
	foreach(new i : Plants)
	{
...
Plant_Refresh(id)
{
	if(id != -1)
    {
		...
		switch(PlantData[id][PlantType])
		{
			case 1:
			{
				zOffset = (PlantData[id][PlantTime] > 1800) ? -1.5 : ((PlantData[id][PlantTime] > 10) ? -1.3 : -1.0);
				objID = CreateDynamicObject(760, PlantData[id][PlantX], PlantData[id][PlantY], PlantData[id][PlantZ] + zOffset, 0.0, 0.0, 0.0, -1, -1, -1, 500.0, 500.0, -1, 0);
			}
			case 2:
			{
				zOffset = (PlantData[id][PlantTime] > 1800) ? -1.0 : ((PlantData[id][PlantTime] > 10) ? -0.5 : 0.0);
				objID = CreateDynamicObject(804, PlantData[id][PlantX], PlantData[id][PlantY], PlantData[id][PlantZ] + zOffset, 0.0, 0.0, 0.0, -1, -1, -1, 500.0, 500.0, -1, 0);
			}
			case 3:
			{
				zOffset = (PlantData[id][PlantTime] > 1800) ? -1.0 : ((PlantData[id][PlantTime] > 10) ? -0.7 : -0.4);
				objID = CreateDynamicObject(949, PlantData[id][PlantX], PlantData[id][PlantY], PlantData[id][PlantZ] + zOffset, 0.0, 0.0, 0.0, -1, -1, -1, 500.0, 500.0, -1, 0);
			}
			case 4:
			{
				zOffset = (PlantData[id][PlantTime] > 1800) ? -2.0 : ((PlantData[id][PlantTime] > 10) ? -1.5 : -1.0);
				objID = CreateDynamicObject(3409, PlantData[id][PlantX], PlantData[id][PlantY], PlantData[id][PlantZ] + zOffset, 0.0, 0.0, 0.0, -1, -1, -1, 500.0, 500.0, -1, 0);
			}
		}

		cpID = CreateDynamicCP(PlantData[id][PlantX], PlantData[id][PlantY], PlantData[id][PlantZ], 2.0, 0, 0, -1, 2.0);

		PlantData[id][PlantObjID] = objID;
		PlantData[id][PlantCP] = cpID;
	}
}
...
// 		{
// 			DestroyDynamicObject(PlantData[id][PlantObjID]);
// 			PlantData[id][PlantObjID] = STREAMER_TAG_OBJECT: -1;
// 		}
// 		if(IsValidDynamicCP(PlantData[id][PlantCP]))
// 		{
// 			DestroyDynamicCP(PlantData[id][PlantCP]);
...
// 		// Simpan posisi dan jenis tanaman
// 		new objID, cpID;
// 		new Float: zOffset;
	...
	if(pData[playerid][pHarvestID] != -1)
	{
		...
		if(pData[playerid][pActivityTime] >= 100)
		{
			...
			else if(PlantData[id][PlantType] == 4)
			{
				...
				Info(playerid, "Anda mendapatkan hasil panen marijuana/ganja seberat "GREEN_E"%d g.", kg);
			}
...
CMD:plant(playerid, params[])
{
	...
	if(isnull(params)) return SyntaxMsg(playerid, "/plant [plant/harvest/destroy/sell]");
	...
	else if(!strcmp(params, "sell", true))
	{
		...
		Info(playerid, "You selling "RED_E"%d g "WHITE_E"all plant to "GREEN_E"%s", total, FormatMoney(pay));
	}
	return 1;
}
...
CMD:sellmarijuana(playerid, params[])
{
	...
	Info(playerid, "You selling "RED_E"%d g "WHITE_E"marijuana to "GREEN_E"%s", total, FormatMoney(pay));
...
CMD:getjobfarmer(playerid, params[])
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, -383.67, -1438.90, 26.32)) return SendClientMessage(playerid, -1, "{FF0000}[INFO JOB] > {CECECE}Untuk memulai hari kerja sebagai Farmer, Anda harus berada di Ladang");
	{
		if(pData[playerid][pJobTime] > 0) return Error(playerid, "Anda harus menunggu "GREY2_E"%d "WHITE_E"detik untuk bisa bekerja kembali.", pData[playerid][pJobTime]);
		{
			if (pData[playerid][pJob] > 0) 
			{
				...
				{
					SuccesMsg(playerid, "Anda Memilih Pekerjaan Sebagai ~p~Farmer");
					pData[playerid][pJob2] = 6; 
				}
			} 
			else 
			{
...
CMD:endjobfarmer(playerid, params[])
{
    if(!IsPlayerInRangeOfPoint(playerid, 3.0, -383.67, -1438.90, 26.32)) 
    {
        return SendClientMessage(playerid, COLOR_GREY, "Anda harus di Tempat Pemberhentian Kerja Farmer!");
    }
    if (pData[playerid][pJob] == 6 || pData[playerid][pJob2] == 6)
    {
        ...
        if (pData[playerid][pJob2] == 6) 
        {
            pData[playerid][pJob2] = 0;
        }

        SuccesMsg(playerid, "Anda resign menjadi ~p~Farmer");
    }
    else
    {
        Error(playerid, "Anda tidak memiliki pekerjaan sebagai ~p~Farmer.");
    }
