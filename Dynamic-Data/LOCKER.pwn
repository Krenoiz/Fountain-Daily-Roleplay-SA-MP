...
Locker_Refresh(lid)
{
    if(lid != -1)
    {
		...
		format(string, sizeof(string), "[ID: %d]\n"WHITE_E"Type: "YELLOW_E"%s\n Tekan "LG_E"Y {FFFFFF}untuk mengakses locker", lid, type);
...
CMD:createlocker(playerid, params[])
{
	...
	new type;
	if(sscanf(params, "d", type)) return SendClientMessageEx(playerid, COLOR_WHITE, "[USAGE]: /createlocker [type, 1.PD 2.MD 3.GS 4.NA 5.GOCAR 6.VIP]");
...
CMD:editlocker(playerid, params[])
{
    ...
    else if(!strcmp(type, "type", true))
    {
        ...
        if(sscanf(string, "d", tipe))
            return SendClientMessageEx(playerid, COLOR_WHITE, "[USAGE]: /editlocker [id] [type] [type, 1.PD 2.MD 3.GS 4.NA 5.GOCAR 6.VIP]");
