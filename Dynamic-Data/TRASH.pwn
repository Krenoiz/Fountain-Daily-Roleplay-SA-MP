...
CMD:edittrash(playerid, params[])
{
	...
	new id;
	if(sscanf(params, "i", id)) return SyntaxMsg(playerid, "/edittrash [id]");
	if(!Iter_Contains(Trashs, id)) return Error(playerid, "Invalid ID.");
	if(!IsPlayerInRangeOfPoint(playerid, 5.0, TrashData[id][TrashX], TrashData[id][TrashY], TrashData[id][TrashZ])) return Error(playerid, "Kamu tidak berada di dekat tempat sampah.");
	pData[playerid][EditingTrash] = id;
	EditDynamicObject(playerid, TrashData[id][TrashPickup]);

	new query[256];
    mysql_format(g_SQL, query, sizeof(query), "UPDATE trash SET posx='%f', posy='%f', posz='%f', posa='%f' WHERE ID='%d'",
    TrashData[id][TrashX], TrashData[id][TrashY], TrashData[id][TrashZ], TrashData[id][TrashA], id);
...
CMD:createtrash(playerid, params[])
{
	...
	if(id == -1) return Error(playerid, "Can't add any more Trash Point.");
...
CMD:gototrash(playerid, params[])
{
	...
	if(!Iter_Contains(Trashs, id)) return Error(playerid, "Trash Point ID tidak ada.");
