...
Garkot_Refresh(id)
{
	if(id != -1)
	{
		...
		new str[128];
		format(str, sizeof(str), "[ID: %d]\n{fa0093}%s", id, GetLocation(gkData[id][gkX], gkData[id][gkY], gkData[id][gkZ]));
		gkData[id][parkLabel] = CreateDynamic3DTextLabel(str, COLOR_GREEN, gkData[id][gkX], gkData[id][gkY], gkData[id][gkZ] + 0.3, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, gkData[id][gkVW], gkData[id][gkInt], -1, 10.0);
		...
		foreach(new id : Garkot)
		{
			if(IsPlayerInDynamicArea(playerid, gkData[id][gkArea]))
			{
				if(areaid == gkData[id][gkArea])
				{
					...
					InfoMsg(playerid, "Tekan ATL Untuk mengakses");
				}
			}
		}
	}
...
CMD:creategarkot(playerid, params[])
{
	...
	if(gkid == -1)
		return Error(playerid, "You cant create more garkot point!");
...
CMD:editposisi(playerid, params[])
{
    ...
    {
		if(!IsPlayerInRangeOfPoint(playerid, 5.0, gkData[gkid][gkX], gkData[gkid][gkY],gkData[gkid][gkZ])) return Error(playerid, "Kamu tidak berada di dekat garasi kota itu.");
    ...
    {
		if(!IsPlayerInRangeOfPoint(playerid, 5.0, gkData[gkid][sgkX], gkData[gkid][sgkY],gkData[gkid][sgkZ])) return Error(playerid, "Kamu tidak berada di dekat garasi kota itu.");
		pData[playerid][EditingGarkot] = gkid;
			...
			foreach(new ii : PVehicles)
			{
				if(vehid == pvData[ii][cVeh])
				{
					if(pvData[ii][cOwner] == pData[playerid][pID])
					{
						...
						SetVehicleVirtualWorld(pvData[vid][cVeh], random(99999) + 1);
	...
	foreach(new gkid : Garkot)
	{
		if(IsPlayerInRangeOfPoint(playerid, 3.5, gkData[gkid][gkX], gkData[gkid][gkY],gkData[gkid][gkZ]))
		{
			...
			foreach(new i : PVehicles)
			{
				...
				{
					if(gkData[gkid][gkX] == 0) return Error(playerid, "garkot point ini belum ada spawn point!");
					...
					if(pvData[i][cPark] == gkid && pvData[i][cClaim] == 0)
					{
						...
						SetVehicleVirtualWorld(pvData[vid][cVeh], 1);
					}
					new string[1024];
...
CMD:gotogarkot(playerid, params[])
{
	...
	if(!Iter_Contains(Garkot, gkid))
		return Error(playerid, "Invalid garkot ID!");
