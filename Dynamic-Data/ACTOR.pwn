...
{
	new cQuery[512];
	format(cQuery, sizeof(cQuery), "UPDATE `actor` SET `name` = '%s', `skin` = '%d', `anim` = '%d', `posx` = '%f', `posy` = '%f', `posz` = '%f', `posr` = '%f', `interior` = '%d', `world` = '0' WHERE `ID` = '%d'",
...
CMD:createactor(playerid, params[])
{
	...
	Iter_Add(Actors, tid);
	mysql_format(g_SQL, query, sizeof query, "INSERT INTO actor SET ID = '%d', skin = '%d', name = '%s', anim = '%d', posx = '%f', posy = '%f', posz = '%f', posr = '%f', interior = '%d', world = '0'", ActorData[tid][actorID], ActorData[tid][actorSkin], ActorData[tid][actorName], ActorData[tid][actorAnim], ActorData[tid][actorX], ActorData[tid][actorY], ActorData[tid][actorZ], ActorData[tid][actorR], ActorData[tid][actorInt], ActorData[tid][actorWorld]);
