...
Player_OwnsHouse(playerid, houseid)
{
    ...
    return (hData[houseid][hOwnerID] == pData[playerid][pID]) ||
           (!strcmp(hData[houseid][hOwner], pData[playerid][pName], true));
}	
// Player_OwnsHouse(playerid, houseid)
// {
...
Player_OwnsHouser(playerid, houseid)
{
	...
	if(!strcmp(hData[houseid][hOwnern], pData[playerid][pName], true)) return 1;
	return 0;
}
...
HouseReset(houseid)
{
    ...
    hData[houseid][hClipA] = 0;
    hData[houseid][hClipB] = 0;
    hData[houseid][hClipC] = 0;
	House_Type(houseid);
...
{
	new cQuery[1536];
	format(cQuery, sizeof(cQuery), "UPDATE houses SET owner='%s', ownern='%s', ownerid='%d', address='%s', price='%d', type='%d', locked='%d', money='%d', clipa='%d', clipb='%d', clipc='%d'",
	...
	hData[houseid][hClipA],
	hData[houseid][hClipB],
	hData[houseid][hClipC]
	);
...
House_Type(houseid)
{
	...
	{
		hData[houseid][hIntposX] = 2259.5386;
		hData[houseid][hIntposY] = -1135.7291;
		hData[houseid][hIntposZ] = 1050.6328;
		hData[houseid][hIntposA] = 273.1317;
		hData[houseid][hInt] = 10;
	    /* switch(random(3))
		{
			case 0:
			{
				hData[houseid][hIntposX] = -42.5248;
				hData[houseid][hIntposY] = 1405.9797;
				hData[houseid][hIntposZ] = 1084.4297;
				hData[houseid][hIntposA] = 178.3122;
				hData[houseid][hInt] = 8;
			}
			case 1:
			{
				hData[houseid][hIntposX] = 221.9200;
				hData[houseid][hIntposY] = 1140.2000;
				hData[houseid][hIntposZ] = 1082.6094;
				hData[houseid][hIntposA] = 184.4772;
				hData[houseid][hInt] = 4;
			}
			case 2:
			{
				hData[houseid][hIntposX] = 260.8500;
				hData[houseid][hIntposY] = 1237.2400;
				hData[houseid][hIntposZ] = 1084.2578;
				hData[houseid][hIntposA] = 179.9571;
	...
	{
		hData[houseid][hIntposX] = 1542.7527;
		hData[houseid][hIntposY] = 1440.7672;
		hData[houseid][hIntposZ] = 1011.3362;
		hData[houseid][hIntposA] = 184.0425;
		hData[houseid][hInt] = 1;
	    /* switch(random(2))
		{
			case 0:
			{
				hData[houseid][hIntposX] = 1430.3329;
				hData[houseid][hIntposY] = -885.8291;
				hData[houseid][hIntposZ] = 1550.6563;
				hData[houseid][hIntposA] = 0.5142;
				hData[houseid][hInt] = 1;
			}
			case 1:
			{
	...
	{
		hData[houseid][hIntposX] = 1430.7488;
		hData[houseid][hIntposY] = 1640.5187;
		hData[houseid][hIntposZ] = 912.8125;
		hData[houseid][hIntposA] = 0.9759;
		hData[houseid][hInt] = 1;
	}
	if(hData[houseid][hType] == 4)
	{
		hData[houseid][hIntposX] = 243.9230;
		hData[houseid][hIntposY] = 304.8654;
		hData[houseid][hIntposZ] = 999.1484;
		hData[houseid][hIntposA] = 271.6749;
		hData[houseid][hInt] = 1;
	}	
...
House_Refresh(houseid)
{
    if(houseid != -1)
    {
        if(IsValidDynamic3DTextLabel(hData[houseid][hLabel]))
		{
			...
			hData[houseid][hLabel] = STREAMER_TAG_3D_TEXT_LABEL: -1;
		}
		...
		{
			format(string, sizeof(string), "[ID: %d]\n{FFFFFF}House Location {FFFF00}%s\n{FFFFFF}House Type {FFFF00}%s\n"WHITE_E"Owned by %s\nTax Time: %s\nPress '{FF0000}ENTER{FFFFFF}' to enter", houseid, GetLocation(hData[houseid][hExtposX], hData[houseid][hExtposY], hData[houseid][hExtposZ]), type, hData[houseid][hOwner], ReturnTimelapse(gettime(), hData[houseid][hVisit]));
 	...
 	if(rows)
  	{
		for(new i; i < rows; i++)
		{
			...
			cache_get_value_name_int(i, "clipa", hData[hid][hClipA]);
			cache_get_value_name_int(i, "clipb", hData[hid][hClipB]);
			cache_get_value_name_int(i, "clipc", hData[hid][hClipC]);
...
GetHouseStorage(houseid, item)
{
	static const StorageLimit[][] = {
	   //Snack  Sprunk  Medicine  Medkit  Bandage  Seed  Material  Component  Marijuana
	    {30,    30,     25,  	  5,  	  10,  	   250,  500,		500, 	 100},// Small
		{50,    50,     35,  	  15,  	  25,  	   300,  1000,  	1000, 	 200},// Medium
		{75,    75,     50,  	  30,  	  50,  	   450,  2000,		2000, 	 300},// Large
		{20,    20,     15,  	  3,  	  5,  	   150,  500,  	    150, 	 50}// Very Small
	};
...
CMD:edithouse(playerid, params[])
{
	...
	else if(!strcmp(type, "type", true))
    {
        ...
        if(sscanf(string, "d", htype))
            return Usage(playerid, "/edithouse [id] [Type] [1.small 2.medium 3.Big 4. Apartment]");
    ...
    else if(!strcmp(type, "owner", true))
    {
		...
		hData[hid][hVisit] = gettime() + (86400 * 7);
...
CMD:lockhouse(playerid, params[])
{
	foreach(new hid : Houses)
	{
		if(IsPlayerInRangeOfPoint(playerid, 10.0, hData[hid][hIntposX], hData[hid][hIntposY], hData[hid][hIntposZ]) || IsPlayerInRangeOfPoint(playerid, 2.5, hData[hid][hExtposX], hData[hid][hExtposY], hData[hid][hExtposZ]))
		{
			if(!Player_OwnsHouse(playerid, hid)) return Error(playerid, "You don't own this house / you are in the interior of your house.");
...
CMD:givehouse(playerid, params[])
{
	...
	if(hid == -1) return Error(playerid, "Invalid id");
	if(pData[otherid][pMaskOn] == 1) return Error(playerid, "Tidak boleh menggunakan mask saat membeli rumah.");
	if(!IsPlayerConnected(otherid) || !NearPlayer(playerid, otherid, 4.0))
        return Error(playerid, "The specified player is disconnected or not near you.");
	...
	{
		#if LIMIT_PER_PLAYER > 0
		if(Player_HouseCount(otherid) + 1 > 10) return Error(playerid, "Target player cant own any more houses.");
		#endif
	}
	else
	{
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1392.5812, -12.6790, 1000.9166)) return Error(playerid, "Anda harus berada di Pemerintah!");
...
CMD:hm(playerid, params[])
{
    ...
    if (pData[playerid][pInHouse] < 0 || pData[playerid][pInHouse] >= MAX_HOUSES) {
        return Error(playerid, "ID rumah tidak valid.");
    }
