...
Doors_Updatelabel(id)
{
	if(id != -1)
	{
		...
		{
			new mstr[512];
			format(mstr,sizeof(mstr),"{BABABA}No %d\n{BABABA}%s", id, dData[id][dName]);
			dData[id][dPickupext] = CreateDynamicPickup(19197, 23, dData[id][dExtposX], dData[id][dExtposY], dData[id][dExtposZ], dData[id][dExtvw], dData[id][dExtint], -1, 50);
        ...
        if(dData[id][dIntposX] != 0.0 && dData[id][dIntposY] != 0.0 && dData[id][dIntposZ] != 0.0)
        {
			if(dData[id][dGarage] == 1)
			{
				...
				dData[id][dPickupint] = CreateDynamicPickup(19197, 23, dData[id][dIntposX], dData[id][dIntposY], dData[id][dIntposZ], dData[id][dIntvw], dData[id][dIntint], -1, 50);
			}
			else
			{
				new mstr[512];
