...
Bisnis_Reset(id)
{
    ...
    format(bizEmploy[id][0], MAX_PLAYER_NAME, "-");
    format(bizEmploy[id][1], MAX_PLAYER_NAME, "-");
    format(bizEmploy[id][2], MAX_PLAYER_NAME, "-");
	bData[id][bOwnerID] = 0;
	bData[id][bLocked] = 1;
    bData[id][bMoney] = 0;
	bData[id][bProd] = 0;
    ...
    for(new z = 0; z < 3; z++)
    {
        format(bizEmploy[id][z], MAX_PLAYER_NAME, "-");
    }
	Bisnis_Refresh(id);
}

IsBusinessEmploye(playerid, id)
{
    if(!strcmp(bizEmploy[id][0], pData[playerid][pName], true)) return 1;
    if(!strcmp(bizEmploy[id][1], pData[playerid][pName], true)) return 1;
    if(!strcmp(bizEmploy[id][2], pData[playerid][pName], true)) return 1;
...
Bisnis_BuyMenu(playerid, bizid)
{
    ...
    switch(bData[bizid][bType])
    {
        ...
        case 2:
        {
			format(string, sizeof(string), "Produk\tHarga\n{ffffff}Snack\t{7fff00}%s\n{ffffff}Sprunk\t{7fff00}%s\n{ffffff}Gas Fuel\t{7fff00}%s\n{ffffff}Bandage\t{7fff00}%s\n{ffffff}Dice\t{7fff00}%s\n{ffffff}Ticket Lottery\t{7fff00}%s\n{ffffff}Tol Card (10x)\t{7fff00}%s\n{ffffff}Cigar(12 pcs)\t{7fff00}%s\n",
                FormatMoney(bData[bizid][bP][0]),
                FormatMoney(bData[bizid][bP][1]),
                FormatMoney(bData[bizid][bP][2]),
                FormatMoney(bData[bizid][bP][3]),
		...
		case 6: // GYM
        {
            format(string, sizeof(string), "Produk\tHarga\n{ffffff}GYM Membership\t{7fff00}%s\n{ffffff}Sprunk\t{7fff00}%s\n{ffffff}Snack\t{7fff00}%s\n",
                FormatMoney(bData[bizid][bP][0]),
                FormatMoney(bData[bizid][bP][1]),
                FormatMoney(bData[bizid][bP][2])
            );
...
Bisnis_Type(bisid)
{
	...
	{
		bData[bisid][bIntposX] = 1542.8801;
		bData[bisid][bIntposY] = 1172.9104;
		bData[bisid][bIntposZ] = 1011.4443;
		bData[bisid][bIntposA] =  90.7687;
		bData[bisid][bInt] = 1;
	}
	if(bData[bisid][bType] == 6) // GYM
	{
		bData[bisid][bIntposX] = 772.3700;
		bData[bisid][bIntposY] = -5.5100;
		bData[bisid][bIntposZ] = 1000.7286;
		bData[bisid][bIntposA] =  354.2480;
		bData[bisid][bInt] = 5;
	}
...
Bisnis_Refresh(id)
{
    if(id != -1)
    {
		...
		{
			bData[id][bMap] = CreateDynamicMapIcon(bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ], 54, -1, -1, -1, -1, 70.0);
		}
		else
		{
			DestroyDynamicMapIcon(bData[id][bMap]);
 	...
 	if(rows)
  	{
		for(new i; i < rows; i++)
		{
            ...
            for(new z = 0; z < 3; z++)
            {
                ...
                cache_get_value_name(i, str, bizEmploy[bid][z]);
            }
		...
		foreach(new i : Bisnis)
		{
			...
			{
				new query[128];
				mysql_format(g_SQL, query, sizeof query, "SELECT * FROM `gymobjects` WHERE `BizID` = '%d'", i);
				mysql_tquery(g_SQL, query, "OnGYMObjectLoaded", "d", i);
			}
		}
...
CMD:createbiz(playerid, params[])
{
	...
	new price, type;
	if(sscanf(params, "dd", price, type)) return Usage(playerid, "/createbusiness [price] [type, 1.Fastfood 2.Market 3.Clothes 4.Equipment 5.Electronics 6.GYM]");
    ...
    format(bizEmploy[bid][0], MAX_PLAYER_NAME, "-");
    format(bizEmploy[bid][1], MAX_PLAYER_NAME, "-");
    format(bizEmploy[bid][2], MAX_PLAYER_NAME, "-");
	bData[bid][bPh] = 0;
	Iter_Add(Bisnis, bid);

	mysql_format(g_SQL, query, sizeof(query), "INSERT INTO bisnis SET ID='%d', owner='%s', ownerid='%d', price='%d', type='%d', extposx='%f', extposy='%f', extposz='%f', extposa='%f', name='%s'", bid, bData[bid][bOwner], bData[bid][bOwnerID], bData[bid][bPrice], bData[bid][bType], bData[bid][bExtposX], bData[bid][bExtposY], bData[bid][bExtposZ], bData[bid][bExtposA], bData[bid][bName], bData[bid][bPh]);
...
CMD:editbiz(playerid, params[])
{
	...
	else if(!strcmp(type, "type", true))
    {
        ...
        if(sscanf(string, "d", btype))
            return Usage(playerid, "/editbisnis [id] [Type] [1.Fastfood 2.Market 3.Clothes 4.Equipment 5.Electronics 6.GYM]");
...
{
	if(GetOwnedBisnis(playerid) == 0) return Error(playerid, "Anda tidak memiliki bisnis sama sekali.");
...
CMD:givebusiness(playerid, params[])
{
	...
	{
		#if LIMIT_PER_PLAYER > 0
		if(Player_BisnisCount(otherid) + 1 > 10) return Error(playerid, "Target player cant own any more houses.");
		#endif
	}
	else
	{
...
GetBusinessType(bizid) {
	...
	pickup = "None";
    switch (bData[bizid][bType]) {
        case 1: pickup = "Fast Food";
        case 2: pickup = "24/7 Market";
        case 3: pickup = "Clothes";
        case 4: pickup = "Equipment";
        case 5: pickup = "Electronic";
    }
    return pickup;
}
