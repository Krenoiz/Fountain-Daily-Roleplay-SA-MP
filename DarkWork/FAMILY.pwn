 	...
 	if(rows)
  	{
		...
		for(new i; i < rows; i++)
		{
			...
			cache_get_value_name_int(i, "point", fData[fid][fPoint]);
...
CMD:fcreate(playerid, params[])
{
	...
	SendStaffMessage(COLOR_LIGHTRED, "Admin %s telah membuat family ID: %d dengan leader: %s", pData[playerid][pAdminname], fid, pData[otherid][pName]);
	new str[150];
...
CMD:fdelete(playerid, params[])
{
	...
	mysql_tquery(g_SQL, query);
    SendStaffMessage(COLOR_LIGHTRED, "Admin %s telah menghapus family ID: %d.", pData[playerid][pAdminname], fid);
	new str[150];
...
CMD:fedit(playerid, params[])
{
    ...
    if(!strcmp(type, "location", true))
    {
        ...
        SendStaffMessage(COLOR_LIGHTRED, "%s has adjusted the location of entrance ID: %d.", pData[playerid][pAdminname], fid);
    }
    else if(!strcmp(type, "interior", true))
    {
        ...
        SendStaffMessage(COLOR_LIGHTRED, "%s has adjusted the interior spawn of entrance ID: %d.", pData[playerid][pAdminname], fid);
    }
    else if(!strcmp(type, "name", true))
    {
        ...
        SendStaffMessage(COLOR_LIGHTRED, "Admin %s has changed the family name ID: %d to: %s.", pData[playerid][pAdminname], fid, name);
    }
    else if(!strcmp(type, "leader", true))
    {
        ...
        SendStaffMessage(COLOR_LIGHTRED, "Admin %s has changed the family leader ID: %d to: %s.", pData[playerid][pAdminname], fid, pData[otherid][pName]);
    }
    else if(!strcmp(type, "safe", true))
    {
		...
		SendStaffMessage(COLOR_LIGHTRED, "Admin %s has changed the family safepos ID: %d.", pData[playerid][pAdminname], fid);
    }
    else if(!strcmp(type, "money", true))
    {
		...
		SendStaffMessage(COLOR_LIGHTRED, "Admin %s has changed the family money ID: %d to %s.", pData[playerid][pAdminname], fid, FormatMoney(money));
    }
    else if(!strcmp(type, "marijuana", true))
    {
		...
		SendStaffMessage(COLOR_LIGHTRED, "Admin %s has changed the family marijuana ID: %d to %d.", pData[playerid][pAdminname], fid, marijuana);
    }
	else if(!strcmp(type, "component", true))
    {
		...
		SendStaffMessage(COLOR_LIGHTRED, "Admin %s has changed the family component ID: %d to %d.", pData[playerid][pAdminname], fid, comp);
    }
	else if(!strcmp(type, "material", true))
    {
        ...
        fData[fid][fMaterial] = mat;
		
        Family_Save(fid);
		Family_Refresh(fid);
		
		SendStaffMessage(COLOR_LIGHTRED, "Admin %s has changed the family material ID: %d to %d.", pData[playerid][pAdminname], fid, mat);
    }
	else if(!strcmp(type, "point", true))
    {
        ...
        if(sscanf(string, "d", point))
            return Usage(playerid, "/fedit [id] [point] [ammount]");

        fData[fid][fPoint] = point;
		
        Family_Save(fid);
		Family_Refresh(fid);
		
		SendStaffMessage(COLOR_LIGHTRED, "Admin %s has changed the family point ID: %d to %d.", pData[playerid][pAdminname], fid, point);
    }
    return 1;
}
...
CMD:finvite(playerid, params[])
{
	...
	Servers(otherid, "%s telah menginvite anda untuk menjadi anggota family. Type: /accept family !", pData[playerid][pName]);
	return 1;
}
...
CMD:fsetrank(playerid, params[])
{
	...
	if(sscanf(params, "ud", otherid, rank))
        return SyntaxMsg(playerid, "/fsetrank [playerid/PartOfName] [rank 1-6]");
 	...
 	if(rows)
  	{
		...
		cache_get_value_index_int(0, 6, fpoint);

		format(string, sizeof(string), "Family ID: %d\nFamily Name: %s\nFamily Leader: %s\nFamily Marijuana: %d\nFamily Component: %d\nFamily Material: %d\nFamily Money: %s\nFamily Point: %d",
		pData[playerid][pFamily], fname, fleader, fmarijuana, fcomponent, fmaterial, fpoint, FormatMoney(fmoney));
 	...
 	if(rows)
  	{
		for(new i = 0; i != rows; i++)
		{
			...
			if(frank == 1)
			{
				strcat(query, ""FAMILY_E"Lookout(1)");
			}
			else if(frank == 2)
			{
				strcat(query, ""FAMILY_E"Hustlers(2)");
			}
			else if(frank == 3)
			{
				strcat(query, ""FAMILY_E"Street Soldier(3)");
			}
			else if(frank == 4)
			{
				strcat(query, ""FAMILY_E"Warlords(4)");
			}
			else if(frank == 5)
			{
				strcat(query, ""FAMILY_E"Shotcaller(5)");
			}
			else if(frank == 6)
			{
				strcat(query, ""FAMILY_E"Leader(6)");
			}
			else
			{
