...
function Loadrobbery()
{
	...
	if(rows)
	{
	    for(new i; i < rows; i++)
	    {
	        ...
	        new label[200];
	        format(label, sizeof label, "%s", RobberyData[aid][robberyName]);
	        ...
	        RobberyData[aid][robberyText] = CreateDynamic3DTextLabel(label, -1, RobberyData[aid][robberyX], RobberyData[aid][robberyY], RobberyData[aid][robberyZ]+1,10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, RobberyData[aid][robberyWorld], RobberyData[aid][robberyInt], -1);
...
{
	new cQuery[512];
	format(cQuery, sizeof(cQuery), "UPDATE `robbery` SET `name` = '%s', `skin` = '%d', `anim` = '%d', `posx` = '%f', `posy` = '%f', `posz` = '%f', `posr` = '%f', `interior` = '%d', `world` = '0' WHERE `ID` = '%d'",
...
CMD:createrobbery(playerid, params[])
{
	...
	if(tid == -1) return Error(playerid, "robbery Has Reached The Max Number");
	...
	new label[100];
	format(label, sizeof label, "%s", RobberyData[tid][robberyName]);
	...
	mysql_format(g_SQL, query, sizeof query, "INSERT INTO robbery SET ID = '%d', skin = '%d', name = '%s', anim = '%d', posx = '%f', posy = '%f', posz = '%f', posr = '%f', interior = '%d', world = '0'", RobberyData[tid][robberyID], RobberyData[tid][robberySkin], RobberyData[tid][robberyName], RobberyData[tid][robberyAnim], RobberyData[tid][robberyX], RobberyData[tid][robberyY], RobberyData[tid][robberyZ], RobberyData[tid][robberyR], RobberyData[tid][robberyInt], RobberyData[tid][robberyWorld]);
...
CMD:deleterobbery(playerid, params[])
{
	...
	if(!Iter_Contains(robberys, id)) return Error(playerid, "Invalid ID.");
...
CMD:gotorobbery(playerid, params[])
{
	...
	if(!Iter_Contains(robberys, id)) return Error(playerid, "robbery ID tidak ada.");
