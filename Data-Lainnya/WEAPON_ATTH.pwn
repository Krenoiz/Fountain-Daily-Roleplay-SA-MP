...
IsWeaponHideable(weaponid)
    return ((weaponid >= 2 && weaponid <= 15) || (weaponid >= 22 && weaponid <= 24 || weaponid == 28 || weaponid == 32 || weaponid == 25));
...
CMD:weapon(playerid, params[])
{
	...
	else if(!strcmp(name, "give", true))
	{
		...
		if(pData[otherid][pLevel] < 5) 
			return Error(playerid, "Player tersebut belum belum level 5 ke atas");

		if(pData[otherid][pCharacterStory] == 0) 
			return Error(playerid, "Player tersebut belum belum memiliki Story Character!");
...
function gunlenz(playerid)
{
	new weaponid = GetPlayerWeaponEx(playerid);
	new ammo = GetPlayerAmmoEx(playerid);
	GivePlayerWeaponEx(playerid, weaponid, GetPlayerAmmo(playerid) - ammo);
	return 1;
}