...
ShowCharacterList(playerid)
{
	...
	{
	    format(sgstr, sizeof(sgstr), ""LB2_E"%s\t Level:%d\n", PlayerChar[playerid][i], LevelChar[playerid][i]);
		strcat(name, sgstr);
		count++;
	}
	if(count < MAX_CHARS)
		strcat(name, ""LB2_E"Buat Character");
	ShowPlayerDialog(playerid, DIALOG_CHARLIST, DIALOG_STYLE_LIST, "Pilih Karakter", name, "Masuk", "Keluar");
	return 1;
}
/*ShowCharacterList(playerid)
{
